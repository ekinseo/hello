...
public class ParsingDOM {
	...
	public void ParsingDOM(String XMLName) {
	...
	public void readNodes(Node node) {
		...
		for(int loop = 0 ; loop < childCount ; loop++) {
			...
			if(!nodeName.equals("#document") && !nodeName.equals("#text")) {
				addNode(nodeName);
			}
		}
	}
	...
	public void setNodeCountData() {
		...
		for(Object tempKey : keyArray) {
			returnData.append("Element").append(tempKey).append(" Count=").append(elementMap.get(tempKey.toString())).append("<BR>");
		}
	}
